...
public data class Board {
	...
	private fun findPlayerTile(player : Player?) : Index? {
		for (row in 0..ROWS - 1) {
			for (col in 0..COLS - 1) {
				var tile : Tile? = tiles.get(row, col)
				if (tile != null && tile!!.players.contains(player)) {
	...
	public fun potentialMoves() : Set<Move?>? {
		...
		options.add(NoMove())
		var player : Player = players.get(currentPlayer)
		addWalkMoves(options, player)
		return options
	}
	private fun addWalkMoves(options : MutableSet<Move>, player : Player) : Unit {
		for (home : Home in homes) {
			if (home.players.contains(player)) {
				val homeConnection : HomeConnection = homeConnections.get(currentPlayer)
				val connecting : Tile? = tiles.get(homeConnection.row, homeConnection.col)
				if (connecting != null) {
		...
		var tile : Tile = tiles.get(index.row, index.col)!!
		for (direction : Direction in tile.connections) {
			var connectedIndex : Index = direction.apply(index)
			if (valid(connectedIndex)) {
				var connectedTile : Tile = tiles.get(connectedIndex.row, connectedIndex.col)!!
	...
	public fun print(out : Appendable) : Unit {
		for (row in 0..ROWS - 1) {
			for (col in 0..COLS - 1) {
				...
				{
					out?.append(' ')?.append((if (tiles?.get(row, col)?.connectsTo(Direction.NORTH)!!)
						'X'
					else
						' '))?.append(' ')
				}
			...
			for (col in 0..COLS - 1) {
				...
				{
					out?.append((if (tiles?.get(row, col)?.connectsTo(Direction.WEST)!!)
						'X'
					else
						' '))
					out?.append((if (tiles?.get(row, col)?.objective != null)
						'G'
					else
						'X'))
					out?.append((if (tiles?.get(row, col)?.connectsTo(Direction.EAST)!!)
						'X'
					else
						' '))
				}
			...
			for (col in 0..COLS - 1) {
				...
				{
					out?.append(' ')?.append((if (tiles?.get(row, col)?.connectsTo(Direction.SOUTH)!!)
						'X'
					else
						' '))?.append(' ')
				}
